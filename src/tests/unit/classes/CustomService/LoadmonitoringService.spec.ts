...
describe("LoadmonitoringService", () => {
  ...
  describe("constructor", () => {
    ...
    let exec = async () => {
      await beforeExec();
      return new LoadmonitoringService(id, dataStorage);
    };

    it("should create and return new LoadmonitoringService", async () => {
    ...
    it("should properties and fields of created CustomService object", async () => {
      ...
      expect(result.Type).toEqual(CustomServiceType.LoadmonitoringService);
      ...
      //Other properties should be initialized properly
      expect(result.Enabled).toEqual(null);
      expect(result.Tenant).toEqual(null);
      expect(result.AssetIds).toEqual(null);
      expect(result.PowerLosses).toEqual(null);
      expect(result.AlertLimit).toEqual(null);
      expect(result.WarningLimit).toEqual(null);
      expect(result.MailingList).toEqual(null);
      expect(result.Interval).toEqual(null);
      expect(result.WarningPoints).toEqual([]);
      expect(result.AlertPoints).toEqual([]);
      expect(result.PredictedPoints).toEqual([]);
      expect(result.HistoricalPoints).toEqual([]);
      expect(result.PredictedPower).toEqual(0);
      expect(result.PredictedEnergy).toEqual(0);
      expect(result.AlertActive).toEqual(false);
      expect(result.WarningActive).toEqual(false);
    });
  });
  ...
  describe("init", () => {
    ...
    let exec = async () => {
      ...
      setPrivateProperty(loadmonitoringService, "_initialized", initialized);

      let payloadToInitialize = cloneObject(initPayload);
      return loadmonitoringService.init(tickId, payloadToInitialize);
    };

    it("should set initialize flag to true and InitTickId to value of tickId", async () => {
      await exec();

      expect(loadmonitoringService.Initialized).toEqual(true);
      expect(loadmonitoringService.InitTickID).toEqual(tickId);
    });

    it("should set properties according to payload", async () => {
      await exec();

      expect(loadmonitoringService.ID).toEqual(id);
      ...
      //Loadmonitoring data should not have changed
      expect(loadmonitoringService.WarningPoints).toEqual([]);
      expect(loadmonitoringService.AlertPoints).toEqual([]);
      expect(loadmonitoringService.PredictedPoints).toEqual([]);
      expect(loadmonitoringService.HistoricalPoints).toEqual([]);
      expect(loadmonitoringService.PredictedPower).toEqual(0);
      expect(loadmonitoringService.PredictedEnergy).toEqual(0);
      expect(loadmonitoringService.AlertActive).toEqual(false);
      expect(loadmonitoringService.WarningActive).toEqual(false);
    ...
    it("should not change type - even if there is different type in payload", async () => {
      initPayload.serviceType = "fakeType" as any;
    ...
    it("should not set properties according to payload - if service has already been initialized", async () => {
      initialized = true;
      ...
      //All properties should have stayed the same as during constructing the service object
      expect(loadmonitoringService.ID).toEqual(id);
      expect(loadmonitoringService.Type).toEqual(
        CustomServiceType.LoadmonitoringService
      );
      ...
      testPrivateProperty(loadmonitoringService, "_dataStorage", dataStorage);
      expect(loadmonitoringService.InitTickID).toEqual(null);
      expect(loadmonitoringService.LastRefreshTickID).toEqual(null);
  ...
  describe("refresh", () => {
    ...
    let initialPredictedEnergy: number;
    let initialPredictedPower: number;
    let alertActive: boolean;
    let warningActive: boolean;
    let variable2111Data: { [tickId: number]: { value: number } };
    let variable2211Data: { [tickId: number]: { value: number } };
    let variable2311Data: { [tickId: number]: { value: number } };
    let notifyAlertActivationMockFunc: jest.Mock;
    let notifyAlertDeacticationMockFunc: jest.Mock;
    let notifyWarningActivationMockFunc: jest.Mock;
    let notifyWarningDeactivationMockFunc: jest.Mock;
    let getTimeSeriesThrows: boolean;

    //Outputs
    let expectValidRefresh: boolean;
    let expectedErrorText: string | null;
    let expectedHistoryPoints: EnergyPoint[];
    let expectedPredictedPoints: EnergyPoint[];
    let expectedPredictedEnergy: number;
    let expectedPredictedPower: number;
    let expectedWarningPoints: EnergyPoint[];
    let expectedAlertPoints: EnergyPoint[];
    let expectedWarningActive: boolean;
    let expectedAlertActive: boolean;
    let expectedNotifyWarningNumberOfCalls: number;
    let expectedNotifyWarningCallParameters: any[];
    let expectedNotifyAlertNumberOfCalls: number;
    let expectedNotifyAlertCallParameters: any[];
    let expectedNotifyWarningDeactivationNumberOfCalls: number;
    let expectedNotifyWarningDeactivationCallParameters: any[];
    let expectedNotifyAlertDeactivationNumberOfCalls: number;
    let expectedNotifyAlertDeactivationCallParameters: any[];
    ...
    beforeEach(() => {
      ...
      initialPredictedEnergy = 10;
      initialPredictedPower = 60;
      ...
      notifyAlertActivationMockFunc = jest.fn();
      notifyAlertDeacticationMockFunc = jest.fn();
      notifyWarningActivationMockFunc = jest.fn();
      notifyWarningDeactivationMockFunc = jest.fn();

      //Outputs
      expectValidRefresh = true;
      expectedHistoryPoints = [
        {
      ...
      //predicted energy at the end -> 1*(205-140) + 2*(117-106) + 3*(164-120) + 7*2/minute + 3*17/minute => 284
      expectedPredictedEnergy = 284;
      //predicted power -> 284 * 6 = 1704
      expectedPredictedPower = 1704;

      //warning consumption per step - > 1500 / 60 = 25
      expectedWarningPoints = [
        {
          tickId: 1618473600000,
          value: 0,
        },
      ...
      //alert consumption per step - > 2100 / 60 = 35
      expectedAlertPoints = [
        {
          tickId: 1618473600000,
          value: 0,
      ...
      //Predicted power: 1704
      //alertLimit: 2100
      //warningLimit: 1500

      //Warning should have been activated
      //Alert should not have been activated
      expectedWarningActive = true;
      expectedAlertActive = false;
      expectedNotifyWarningNumberOfCalls = 1;
      expectedNotifyWarningCallParameters = [tickId, 1704];
      expectedNotifyAlertNumberOfCalls = 0;
      expectedNotifyAlertCallParameters = [];
      expectedNotifyWarningDeactivationNumberOfCalls = 0;
      expectedNotifyWarningDeactivationCallParameters = [];
      expectedNotifyAlertDeactivationNumberOfCalls = 0;
      expectedNotifyAlertDeactivationCallParameters = [];
    ...
    let exec = async () => {
      timeseriesServiceContent["testTenant2"]["asset21"]["aspect211"][
        "variable2111"
      ] = variable2111Data;
      timeseriesServiceContent["testTenant2"]["asset22"]["aspect221"][
        "variable2211"
      ] = variable2211Data;
      timeseriesServiceContent["testTenant2"]["asset23"]["aspect231"][
        "variable2311"
      ] = variable2311Data;
      ...
      setPrivateProperty(
        ...
        notifyAlertActivationMockFunc
      );
      setPrivateProperty(
        loadmonitoringService,
        "_notifyAlertDeactivation",
        notifyAlertDeacticationMockFunc
      );
      setPrivateProperty(
        loadmonitoringService,
        "_notifyWarningActivation",
        notifyWarningActivationMockFunc
      );
      setPrivateProperty(
        loadmonitoringService,
        "_notifyWarningDeactivation",
        notifyWarningDeactivationMockFunc
      );

      setPrivateProperty(
        loadmonitoringService,
        "_predictedEnergy",
        initialPredictedEnergy
      );
      setPrivateProperty(
        loadmonitoringService,
        "_predictedPower",
        initialPredictedPower
      );
      ...
      if (getTimeSeriesThrows) {
        MindSphereTimeSeriesService.getInstance().getValues = jest.fn(
          async () => {
            throw new Error("Test get values error");
          }
        );
      }

      return loadmonitoringService.refresh(tickId);
    };

    let testRefresh = async () => {
      if (expectValidRefresh) {
        await exec();

        //#region ===== CHECKING REFRESH TICK ID ======

        expect(loadmonitoringService.LastRefreshTickID).toEqual(tickId);
        //#endregion ===== CHECKING REFRESH TICK ID ======

        //#region ===== CHECKING HISTORY POINTS ======

        expect(loadmonitoringService.HistoricalPoints).toEqual(
          expectedHistoryPoints
        );

        //#endregion ===== CHECKING HISTORY POINTS ======

        //#region ===== CHECKING PREDICTED POINTS ======

        expect(loadmonitoringService.PredictedPoints).toEqual(
          expectedPredictedPoints
        );

        //#endregion ===== CHECKING PREDICTED POINTS ======

        //#region ===== CHECKING PREDICTED ENERGY AND POWER ======

        expect(loadmonitoringService.PredictedEnergy).toEqual(
          expectedPredictedEnergy
        );
        expect(loadmonitoringService.PredictedPower).toEqual(
          expectedPredictedPower
        );

        //#endregion ===== CHECKING PREDICTED ENERGY AND POWER ======

        //#region ===== CHECKING WARNING AND ALERT POINTS ======

        expect(loadmonitoringService.WarningPoints).toEqual(
          expectedWarningPoints
        );

        expect(loadmonitoringService.AlertPoints).toEqual(expectedAlertPoints);

        //#endregion ===== CHECKING WARNING AND ALERT POINTS ======

        //#region ===== CHECKING WARNING AND ALERT ======
        ...
        expect(loadmonitoringService.WarningActive).toEqual(
          expectedWarningActive
        );
        expect(notifyWarningActivationMockFunc).toHaveBeenCalledTimes(
          expectedNotifyWarningNumberOfCalls
        );
        if (expectedNotifyWarningNumberOfCalls > 0) {
          expect(notifyWarningActivationMockFunc.mock.calls).toContainEqual(
            expectedNotifyWarningCallParameters
          );
        }
        expect(notifyWarningDeactivationMockFunc).toHaveBeenCalledTimes(
          expectedNotifyWarningDeactivationNumberOfCalls
        );
        if (expectedNotifyWarningDeactivationNumberOfCalls > 0) {
          expect(notifyWarningDeactivationMockFunc.mock.calls).toContainEqual(
            expectedNotifyWarningDeactivationCallParameters
          );
        }

        expect(loadmonitoringService.AlertActive).toEqual(expectedAlertActive);
        expect(notifyAlertActivationMockFunc).toHaveBeenCalledTimes(
          expectedNotifyAlertNumberOfCalls
        );
        if (expectedNotifyAlertNumberOfCalls > 0) {
          expect(notifyAlertActivationMockFunc.mock.calls).toContainEqual(
            expectedNotifyAlertCallParameters
          );
        }
        expect(notifyAlertDeacticationMockFunc).toHaveBeenCalledTimes(
          expectedNotifyAlertDeactivationNumberOfCalls
        );
        if (expectedNotifyAlertDeactivationNumberOfCalls > 0) {
          expect(notifyAlertDeacticationMockFunc.mock.calls).toContainEqual(
            expectedNotifyAlertDeactivationCallParameters
          );
        }

        //#endregion ===== CHECKING WARNING AND ALERT ======
      } else {
        await expect(exec()).rejects.toMatchObject({
          message: expectedErrorText,
        });
      }
    };

    it("should refresh the loadmonitoring service and properly set values of historical and predicted points", async () => {
      await testRefresh();
    });

    it("should refresh the loadmonitoring service and properly set values of historical and predicted points - if tickId is a begining of the interval", async () => {
      tickId = 1618473600;
      expectedNotifyWarningCallParameters = [tickId, 1704];
      await testRefresh();
    });

    it("should refresh the loadmonitoring service and properly set values of historical and predicted points - if tickId is the end of the interval", async () => {
      tickId = 1618473600 + 540;
      expectedNotifyWarningCallParameters = [tickId, 1704];
      await testRefresh();
    });

    it("should set empty points and not change predictedEnergy or predictedPower but not throw - if getting one of asset id throws", async () => {
      //Inputs
      getTimeSeriesThrows = true;

      //Outputs
      expectValidRefresh = true;
      //History and predicted points should be empty
      expectedHistoryPoints = [];
      expectedPredictedPoints = [];
      //Energy and power should not have changed
      expectedPredictedEnergy = initialPredictedEnergy;
      expectedPredictedPower = initialPredictedPower;

      //Warning should have been activated
      //Alert should not have been activated
      expectedWarningActive = false;
      expectedAlertActive = false;
      expectedNotifyWarningNumberOfCalls = 0;
      expectedNotifyWarningCallParameters = [];
      expectedNotifyAlertNumberOfCalls = 0;
      expectedNotifyAlertCallParameters = [];
      expectedNotifyWarningDeactivationNumberOfCalls = 0;
      ...
      //logger should be called 3 times - for every call of asset
      expect(logErrorMockFunc).toHaveBeenCalledTimes(3);
      expect(logErrorMockFunc.mock.calls[0][0]).toEqual(
        "Test get values error"
      );
      expect(logErrorMockFunc.mock.calls[1][0]).toEqual(
        "Test get values error"
      );
      expect(logErrorMockFunc.mock.calls[2][0]).toEqual(
        "Test get values error"
      );
    });

    it("should not deactivate warning or alert - if interval changed to new interval", async () => {
      //Inputs
      variable2111Data = {
        "1618473360000": { value: 100 }, //2021-04-15T07:56:00.000Z
        "1618473420000": { value: 105 }, //2021-04-15T07:57:00.000Z
      ...
      initialPredictedEnergy = 300;
      initialPredictedPower = 1800;
    ...
    it("should not deactivate warning or alert - if interval changed to new interval without data", async () => {
      //Inputs
      variable2111Data = {
        "1618473360000": { value: 100 }, //2021-04-15T07:56:00.000Z
        "1618473420000": { value: 105 }, //2021-04-15T07:57:00.000Z
    ...
    it("should reset the loadmonitorign service - if it is disabled", async () => {
      //Inputs
      initPayload.enabled = false;
      ...
      expectedPredictedPoints = [];
      //Energy and power should not have changed
      expectedPredictedEnergy = 0;
      expectedPredictedPower = 0;
    ...
    it("should not refresh the loadmonitoring - if loadmonitoring is not initialzied", async () => {
      initialize = false;
      ...
      expect(loadmonitoringService.WarningActive).toEqual(false);
      expect(notifyWarningActivationMockFunc).not.toHaveBeenCalled();
      expect(notifyWarningDeactivationMockFunc).not.toHaveBeenCalled();

      expect(loadmonitoringService.AlertActive).toEqual(false);
      expect(notifyAlertActivationMockFunc).not.toHaveBeenCalled();
      expect(notifyAlertDeacticationMockFunc).not.toHaveBeenCalled();

      //#endregion ===== CHECKING WARNING AND ALERT ======
    });

    //#region ===== WARNING AND ALERT ACTIVATION / DEACTIVATION =====

    it("should notify warning activation, if predicted power goes above warning limit but below alert limit - service does not have warning or alert active", async () => {
      //Inputs

      //predicted energy at the end -> 1*(205-140) + 2*(117-106) + 3*(164-120) + 7*2/minute + 3*17/minute => 284
      //predicted power -> 284 * 6 = 1704
      initPayload.alertLimit = 2100;
      initPayload.warningLimit = 1500;
    ...
    it("should notify warning activation but not alert deactivation, if predicted power goes above warning limit but below alert limit - service have warning inactive but alert active", async () => {
      //Inputs
    ...
    it("should not notify warning activation, if predicted power goes above warning limit but below alert limit - service has warning and alert inactive", async () => {
      //Inputs
    ...
    it("should notify alert activation but not warning activation, if predicted power goes above alert and warning limit - service does not have warning or alert active", async () => {
      //Inputs

      //predicted energy at the end -> 1*(205-140) + 2*(117-106) + 3*(164-120) + 7*2/minute + 3*17/minute => 284
      //predicted power -> 284 * 6 = 1704
      initPayload.alertLimit = 1500;
      initPayload.warningLimit = 1200;
      ...
      expectedAlertActive = true;
      expectedNotifyAlertNumberOfCalls = 1;
      expectedNotifyAlertCallParameters = [tickId, 1704];
    ...
    it("should not notify alert activation nor warning activation, if predicted power is above alert and warning limit - service has warning inactive and alert active", async () => {
      //Inputs
    ...
    it("should not notify alert activation nor warning activation, if predicted power is above alert and warning limit - service has warning and alert active", async () => {
      //Inputs
    ...
    it("should notify alert activation but not warning activation, if predicted power goes above alert but not warning limit - service does not have warning or alert active", async () => {
      //Inputs

      //predicted energy at the end -> 1*(205-140) + 2*(117-106) + 3*(164-120) + 7*2/minute + 3*17/minute => 284
      //predicted power -> 284 * 6 = 1704
      initPayload.alertLimit = 1500;
      initPayload.warningLimit = 2100;
    ...
    it("should notify alert activation but not warning deactivation, if predicted power goes above alert and warning limit - service has warning active but alert inactive", async () => {
      //Inputs
    ...
    it("should notify alert activation but not warning deactivation, if predicted power goes above alert but not warning limit - service has warning active but alert inactive", async () => {
      //Inputs
    ...
    it("should not notify alert deactivation or warning activation, if predicted power goes above alert and warning limit - service has warning inactive but alert active", async () => {
      //Inputs

      //predicted energy at the end -> 1*(205-140) + 2*(117-106) + 3*(164-120) + 7*2/minute + 3*17/minute => 284
      //predicted power -> 284 * 6 = 1704
      initPayload.alertLimit = 1200;
      initPayload.warningLimit = 1500;
    ...
    it("should notify warning deactivation, if predicted power goes below warning limit and alert limit- service has warning active but not alert active", async () => {
      //Inputs

      //predicted energy at the end -> 1*(205-140) + 2*(117-106) + 3*(164-120) + 7*2/minute + 3*17/minute => 284
      //predicted power -> 284 * 6 = 1704
      initPayload.alertLimit = 2100;
      initPayload.warningLimit = 1800;
      ...
      expectedNotifyWarningDeactivationNumberOfCalls = 1;
      expectedNotifyWarningDeactivationCallParameters = [tickId, 1704];
    ...
    it("should notify alert deactivation, if predicted power goes below warning limit and alert limit- service has warning and alert active", async () => {
      //Inputs
      ...
      expectedNotifyAlertDeactivationNumberOfCalls = 1;
      expectedNotifyAlertDeactivationCallParameters = [tickId, 1704];

      await testRefresh();
    });

    it("should notify alert deactivation, if predicted power goes below alert limit and warning limit- service has warning deactive but alert active", async () => {
      //Inputs
    ...
    it("should not notify alert deactivation or warning activation, if predicted power goes below alert limit but not warning limit- service has warning deactive but alert active", async () => {
      //Inputs
    ...
    it("should not notify alert deactivation or warning activation, if predicted power is below alert limit but not warning limit- service has warning inactive and alert inactive", async () => {
      //Inputs
    ...
    //#endregion ===== WARNING AND ALERT ACTIVATION / DEACTIVATION =====

    it("should refresh the loadmonitoring service and properly set values of historical and predicted points - if there are additional, non-valid tickids", async () => {
      variable2111Data = {
        "1618473360000": { value: 100 }, //2021-04-15T07:56:00.000Z
        "1618473420000": { value: 105 }, //2021-04-15T07:57:00.000Z
        "1618473480000": { value: 120 }, //2021-04-15T07:58:00.000Z
        ...
        "1618473660000": { value: 145 }, //2021-04-15T08:01:00.000Z
        "1618473720000": { value: 160 }, //2021-04-15T08:02:00.000Z
        "1618473730000": { value: 161 }, //2021-04-15T08:02:10.000Z
        "1618473780000": { value: 165 }, //2021-04-15T07:03:00.000Z
        "1618473840000": { value: 180 }, //2021-04-15T07:04:00.000Z
        "1618473900000": { value: 185 }, //2021-04-15T07:05:00.000Z
        "1618473960000": { value: 200 }, //2021-04-15T07:06:00.000Z
      ...
      variable2211Data = {
        ...
        "1618473660000": { value: 108 }, //2021-04-15T08:01:00.000Z
        "1618473720000": { value: 109 }, //2021-04-15T08:02:00.000Z
        "1618473730000": { value: 110 }, //2021-04-15T08:02:10.000Z
        "1618473780000": { value: 111 }, //2021-04-15T07:03:00.000Z
        "1618473840000": { value: 112 }, //2021-04-15T07:04:00.000Z
        "1618473900000": { value: 114 }, //2021-04-15T07:05:00.000Z
        "1618473960000": { value: 115 }, //2021-04-15T07:06:00.000Z
      ...
      variable2311Data = {
        ...
        "1618473660000": { value: 122 }, //2021-04-15T08:01:00.000Z
        "1618473720000": { value: 134 }, //2021-04-15T08:02:00.000Z
        "1618473730000": { value: 135 }, //2021-04-15T08:02:10.000Z
        "1618473780000": { value: 136 }, //2021-04-15T07:03:00.000Z
        "1618473840000": { value: 148 }, //2021-04-15T07:04:00.000Z
        "1618473900000": { value: 150 }, //2021-04-15T07:05:00.000Z
        "1618473960000": { value: 162 }, //2021-04-15T07:06:00.000Z
    ...
    it("should refresh the loadmonitoring service and take into account only existsing samples - if last samples of some variables does not exist", async () => {
      variable2111Data = {
        "1618473360000": { value: 100 }, //2021-04-15T07:56:00.000Z
        "1618473420000": { value: 105 }, //2021-04-15T07:57:00.000Z
        "1618473480000": { value: 120 }, //2021-04-15T07:58:00.000Z
        ...
        "1618473960000": { value: 200 }, //2021-04-15T07:06:00.000Z
        "1618474020000": { value: 205 }, //2021-04-15T07:07:00.000Z
        "1618474080000": { value: 220 }, //2021-04-15T07:08:00.000Z
      };
      variable2211Data = {
        "1618473360000": { value: 100 }, //2021-04-15T07:56:00.000Z
        "1618473420000": { value: 102 }, //2021-04-15T07:57:00.000Z
        ...
        "1618473960000": { value: 115 }, //2021-04-15T07:06:00.000Z
        "1618474020000": { value: 117 }, //2021-04-15T07:07:00.000Z
        //Lack of last sample - 1618474080000 2021-04-15T07:08:00.000Z
      };
      variable2311Data = {
        "1618473360000": { value: 102 }, //2021-04-15T07:56:00.000Z
        "1618473420000": { value: 104 }, //2021-04-15T07:57:00.000Z
        ...
        "1618473960000": { value: 162 }, //2021-04-15T07:06:00.000Z
        "1618474020000": { value: 164 }, //2021-04-15T07:07:00.000Z
        "1618474080000": { value: 176 }, //2021-04-15T07:08:00.000Z
      };
    ...
    it("should not change previous power and energy - if there is no first sample of values", async () => {
      //Inputs
      variable2111Data = {
        "1618473360000": { value: 100 }, //2021-04-15T07:56:00.000Z
        "1618473420000": { value: 105 }, //2021-04-15T07:57:00.000Z
        "1618473480000": { value: 120 }, //2021-04-15T07:58:00.000Z
        "1618473540000": { value: 135 }, //2021-04-15T07:59:00.000Z
        //Lack of first sample 1618473600000 - 2021-04-15T08:00:00.000Z
        "1618473660000": { value: 145 }, //2021-04-15T08:01:00.000Z
        "1618473720000": { value: 160 }, //2021-04-15T08:02:00.000Z
        "1618473780000": { value: 165 }, //2021-04-15T07:03:00.000Z
        "1618473840000": { value: 180 }, //2021-04-15T07:04:00.000Z
    ...
    it("should not change previous power and energy - if there are no samples", async () => {
      //Inputs
      variable2111Data = {
        "1618473360000": { value: 100 }, //2021-04-15T07:56:00.000Z
        "1618473420000": { value: 105 }, //2021-04-15T07:57:00.000Z
    ...
    it("should not change previous power and energy - if there is only one sample", async () => {
      //Inputs
      variable2111Data = {
        "1618473360000": { value: 100 }, //2021-04-15T07:56:00.000Z
        "1618473420000": { value: 105 }, //2021-04-15T07:57:00.000Z
